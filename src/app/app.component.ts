...
export class AppComponent implements OnInit {
  ...
  constructor(private okta : Okta, private changeDetectorRef : ChangeDetectorRef,
    private swUpdate : SwUpdate, private eventBus : EventBusService, private snackBar : MatSnackBar,
    private dataService : DataService, private router : Router, private http : HttpClient) {
      ...
      .get((response) => {
        if (response.status !== 'INACTIVE') {
          ...
          this
            .eventBus
            .message
            .next(this.user + ' is current user.');
        } else {
          this.showLogin();
        }
      });
  ...
  search() {
    ...
    queryString += ') AND ( *' + this.searchString + '* )';
    this
      .http
      .get(this.dataService.baseUrl + '/query/' + queryString)
